...
from llava.eval.att_data import Point, create_data_loader
from llava.eval.att_utils import get_chunk, get_heads
...
def eval_model(args):
    ...
    data_loader = create_data_loader(
        Point, questions, args.image_folder, tokenizer, image_processor, model.config, batch_size=args.batch_size, args=args
    )
    tbar = tqdm(data_loader)
    meta = []
    for i, (batch_mask, input_ids, image_tensors, gt_list, idx_list, attention_mask, image_sizes) in enumerate(tbar):
        meta.extend(gt_list)
    with open(f"{s.name}.json", "w") as f:
        json.dump(meta, f)
