...
def image_preprocess(line, image, image_size_anchor, args):
    strategy = Strategy(args.sname)

    visual_prompt_style  = 'constant'

    line['segmentations'] = [None] * len(line['bboxes'])
    shape_color_info = [["red", (255, 0, 0), "rectangle"]]

    if strategy.highlight == "visual":
        alpha = getattr(args, "alpha", None)
        for instance_idx, (bbox, segmentation) in enumerate(zip(line["bboxes"], line["segmentations"])):
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        processed_image = image_preprocess(line, image, self.image_processor.crop_size["height"], args)
        image_tensor = process_images([processed_image], self.image_processor, self.model_config)[0]
